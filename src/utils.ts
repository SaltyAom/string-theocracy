...
export const removeDuplicateSlashes = (path: string) => {
	...
	let total = newPath.length - 1
	if (newPath.charCodeAt(total) === 47)
		newPath = newPath.substring(0, total)
	if (newPath.charCodeAt(0) === 47) newPath = newPath.substring(1, total + 1)
